...
def produce_signdoc_bytes_for_signing(public_key: bytes, envelope: TerraSignTx, msgs: list[MessageType]) -> bytes:
    ...
    signer_infos = [
        TerraSignerInfo(
            public_key=serialize_to_anytype(TerraPublicKey(value=public_key)),
            mode_info=TerraModeInfo(single=Single(mode=SignMode.SIGN_MODE_DIRECT)),
...
def serialize_to_anytype(msg: MessageType) -> AnyType:
    if TerraPublicKey.is_type_of(msg):
        return AnyType(
            type_url="/cosmos.crypto.secp256k1.PubKey",
            value=dump_message_buffer(msg)
        )
    elif TerraMsgSend.is_type_of(msg):
        return AnyType(
...
def generate_signdoc(envelope: TerraSignTx, tx: TerraTx) -> TerraSignDoc:
    return TerraSignDoc(
