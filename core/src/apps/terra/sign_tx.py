...
async def sign_tx(
    ctx: wire.Context, envelope: TerraSignTx, keychain: Keychain
) -> TerraSignedTx:
    ...
    while len(msgs) < envelope.msg_count:
        msg = await ctx.call_any(
            tx_req,
            MessageType.TerraMsgSend
        )

        if TerraMsgSend.is_type_of(msg):
            await layout.require_confirm_send(ctx, msg, len(msgs) + 1, envelope.msg_count)
        else:
    ...
    await layout.require_confirm_tx(ctx, envelope.msg_count, envelope.fee.amount)
    ...
    return TerraSignedTx(signature=signature_bytes, public_key=node.public_key())
